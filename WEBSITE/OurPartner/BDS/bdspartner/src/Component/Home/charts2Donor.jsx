...
function BloodTypeChart2() {
  ...
  useEffect(() => {
    const fetchRecipientData = async () => {
      const collectionRef = collection(database, 'BDS Recipient');
    ...
    const fetchDonorData = async () => {
      const collectionRef = collection(database, 'BDS Donors');
      const snapshot = await getDocs(collectionRef);
      const docs = snapshot.docs.map(doc => doc.data());
      setDonorData(docs);
    };
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-around' }}>
        <div style={{ padding: '20px', margin: '20px', backgroundColor: '#f5f5f5', borderRadius: '15px' }}>
          ...
          <BarChart width={300} height={300} data={recipientBarChartData}>
        ...
        <div style={{ padding: '20px', margin: '20px', backgroundColor: '#f5f5f5', borderRadius: '15px' }}>
          ...
          <BarChart width={300} height={300} data={donorBarChartData}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="name" />
            <YAxis />
            <Tooltip />
