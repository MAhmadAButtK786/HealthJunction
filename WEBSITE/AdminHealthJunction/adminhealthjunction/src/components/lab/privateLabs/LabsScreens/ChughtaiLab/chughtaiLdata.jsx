...
function Chughtai() {
  ...
  const getData = async () => {
    try {
      const querySnapshot = await getDocs(collection(database, "Chugtai Labs"));
  ...
  const deleteRecipient = async (test) => {
    try {
      await deleteDoc(doc(database, "Chugtai Labs", test.id));
  ...
  return (
    <>
      <div className="flex items-center justify-center">
        <img src="images/img/Lab/chugtai.jpg" alt="Chugtai" className="mx-auto rounded-full w-52 h-52" />
        
      </div>
      <div className="px-8">
      <Link to="/chugtaiinsertpage">
            ...
            <tbody>
              {filteredTests.map((test, index) => (
                <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`}>
                  ...
                  <td className="px-4 py-2 border">
                    <Link to ={`/updateChughtaiPage/${test.id}`}>
